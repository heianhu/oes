...
if (!isset($_SESSION['admname'])) {
    ...
    if (!isset($_GLOBALS['message']) && $hasvar == true)
        $_GLOBALS['message'] = "已成功删除选定的用户";
    else if (!$hasvar) {
        $_GLOBALS['message'] = "首先选择要删除的用户。";
    }
    ...
    if (empty($_REQUEST['cname']) || empty($_REQUEST['password']) || empty($_REQUEST['email'])) {
        ...
        if (!@executeQuery($query)) {
            if (mysql_errno () == 1062) //duplicate value
                $_GLOBALS['message'] = "给定用户名违反一些限制，请尝试使用其他名称。";
            else
                $_GLOBALS['message'] = mysql_error();
        }
        else
            $_GLOBALS['message'] = "已成功创建新用户。";
    }
    closedb();
}
?>
...
?>
                        ...
                        <li><input type="submit" value="测试员" name="tcmng" class="subbtn" title="Test Conductors Management"/></li>
...
?>
                        ...
                        <li><input type="submit" value="保存" name="savea" class="subbtn" onclick="validateform('usermng')" title="Save the Changes"/></li>

<?php
    } else if (isset($_REQUEST['edit'])) { //navigation for Edit option
?>
                        <li><input type="submit" value="取消" name="cancel" class="subbtn" title="Cancel"/></li>
                        <li><input type="submit" value="保存" name="savem" class="subbtn" onclick="validateform('usermng')" title="Save the changes"/></li>
...
<?php
if (isset($_SESSION['admname'])) {
    echo "<div class=\"pmsg\" style=\"text-align:center;\">学生管理 </div>";
    if (isset($_REQUEST['add'])) {
        /*         * ************************ Step 3 - Case 1 ************************ */
